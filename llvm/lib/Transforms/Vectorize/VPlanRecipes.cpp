...
bool VPWidenGEPRecipe::usesFirstLaneOnly(const VPValue *Op) const {
  assert(is_contained(operands(), Op) && "Op must be an operand of the recipe");
  return Op == getOperand(0) && vputils::isSingleScalar(Op);
}

void VPWidenGEPRecipe::execute(VPTransformState &State) {
  assert(State.VF.isVector() && "not widening");
  // A GEP cannot do scalable indexing with a struct type.
  bool ForcedScalar =
      State.VF.isScalable() && getSourceElementType()->isStructTy();
  auto *Ptr = State.get(getOperand(0),
                        ForcedScalar || vputils::isSingleScalar(getOperand(0)));
  auto IdxList = map_to_vector(drop_begin(operands()), [&](VPValue *Op) {
    return State.get(Op, ForcedScalar);
  });
  auto *GEP = State.Builder.CreateGEP(getSourceElementType(), Ptr, IdxList,
                                      "wide.gep", getGEPNoWrapFlags());
  if (ForcedScalar)
    GEP = State.Builder.CreateVectorSplat(State.VF, GEP, "broadcast");
  State.set(this, GEP);
}
