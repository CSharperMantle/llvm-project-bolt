...
; CHECK-NEXT:    [[TMP0:%.*]] = load ptr, ptr [[SRC]], align 8
; CHECK-NEXT:    [[WIDE_GEP:%.*]] = getelementptr [10 x float], ptr [[TMP0]], <4 x i32> [[BROADCAST_SPLAT]], <4 x i64> [[VEC_IND]]
...
; CHECK-NEXT:    [[WIDE_GEP:%.*]] = getelementptr inbounds nuw i8, ptr [[P]], <4 x i64> splat (i64 16)
; CHECK-NEXT:    [[TMP0:%.*]] = trunc <4 x i8> [[BROADCAST_SPLAT]] to <4 x i1>
; CHECK-NEXT:    [[TMP1:%.*]] = select <4 x i1> [[TMP0]], <4 x ptr> [[WIDE_GEP]], <4 x ptr> [[BROADCAST_SPLAT2]]
; CHECK-NEXT:    br label [[VECTOR_BODY:%.*]]
; CHECK:       vector.body:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i32 [ 0, [[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], [[VECTOR_BODY]] ]
