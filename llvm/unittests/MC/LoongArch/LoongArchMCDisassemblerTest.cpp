...
struct Context {
  ...
  Context() {
    ...
    assert(TheTarget && "LoongArch target is not available");
...
class RecordingSymbolizer : public MCSymbolizer {
public:
  struct Call {
  ...
  explicit RecordingSymbolizer(MCContext &Ctx) : MCSymbolizer(Ctx, nullptr) {}
...
TEST(LoongArchDisassembler, ReportsDecodedImmediatesToSymbolizer) {
  auto *Symbolizer = new RecordingSymbolizer(getContext());
  ...
  for (const ImmediateTestCase &TestCase : TestCases) {
    ...
    const RecordingSymbolizer::Call &Call = Symbolizer->Calls.front();
