...
bool RewriteInstance::analyzeRelocation(
  ...
  if (SymbolIter == InputFile->symbol_end()) {
    ...
    if (!SymbolAddress && !IsWeakReference(Symbol) &&
        (IsAArch64 || BC->isRISCV() || IsLoongArch)) {
...
void RewriteInstance::handleRelocation(const SectionRef &RelocatedSection,
  ...
  if (ForceRelocation && !ReferencedBF) {
    ...
    if (BinaryData *BD = BC->getBinaryDataContainingAddress(SymbolAddress)) {
      ...
      // but AArch64, RISCV and LoongArch has inferred and incomplete object
      // locations coming from GOT/TLS or any other non-trivial relocation (that
      // requires creation of sections and whose symbol address is not really
      // what should be encoded in the instruction). So we essentially disabled
      // this check for AArch64 and live with bogus names for objects.
      assert((IsAArch64 || IsRISCV || IsLoongArch || IsSectionRelocation ||
