...
///           predecessor path is a VMEM load (backward CFG walk, depth-bounded).
///
...
public:
  VMemUseOrdering(MachineFunction *MF)
...
static bool isVmemCounterLoad(const MachineInstr &MI,
                               const GCNSubtarget &ST) {
...
// GFX12+: BVH → BVH_CNT; sampler/MSAA IMAGE → SAMPLE_CNT; all others → LOAD_CNT.
...
struct VmemClearedState {
  ...
  void update(const MachineInstr &MI, const GCNSubtarget &ST) {
    ...
    case AMDGPU::S_WAIT_LOADCNT:
      if (Imm == 0) LoadCnt = true;
      break;
    case AMDGPU::S_WAIT_SAMPLECNT:
      if (Imm == 0) SampleCnt = true;
      break;
    case AMDGPU::S_WAIT_BVHCNT:
      if (Imm == 0) BvhCnt = true;
      break;
  ...
  bool clears(AMDGPU::InstCounterType CT) const {
    switch (CT) {
    case AMDGPU::LOAD_CNT:   return LoadCnt;
    case AMDGPU::SAMPLE_CNT: return SampleCnt;
    case AMDGPU::BVH_CNT:    return BvhCnt;
...
static void gatherVMemPendingUnits(
  ...
  for (const MachineBasicBlock *Pred : CurMBB->predecessors()) {
    ...
    if (!Unresolved.empty() && !Cleared.allCleared(ST))
      gatherVMemPendingUnits(Pred, Unresolved, TRI, ST, PendingUnits,
                             Visited, Depth + 1);
  }
}
...
void VMemUseOrdering::apply(ScheduleDAGInstrs *DAG) {
  ...
  if (!MBB->pred_empty()) {
    ...
    if (!Candidates.empty()) {
      ...
      Visited.insert(MBB);
      gatherVMemPendingUnits(MBB, Candidates, *TRI, ST, PendingUnits, Visited, 0);
    }
  }
  ...
  for (SUnit &SU : DAG->SUnits) {
    ...
    LLVM_DEBUG(if (EdgesAdded) dbgs()
               << "VMemUseOrdering: added " << EdgesAdded
               << " edge(s) to SU(" << SU.NodeNum << ") " << *MI);
  }
}
